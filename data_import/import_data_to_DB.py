	...
	for line in aktivnosti_reader:
		if len(line)==5:
			...
			for ime in imena_polj:
				...
				if len(polje_id)==0:
					if mozne_vrednosti:
						conn.execute("INSERT INTO patronazna_sluzba_app_polje_v_porocilu (ime, vnosno_polje, mozne_vrednosti) VALUES (?,?,?)", (ime, vnosno_polje,mozne_vrednosti));
					else:
						conn.execute("INSERT INTO patronazna_sluzba_app_polje_v_porocilu (ime,vnosno_polje) VALUES (?,?)", (ime, vnosno_polje));
					cursor = conn.execute("select id from patronazna_sluzba_app_polje_v_porocilu where ime='" + ime + "' and vnosno_polje='" + vnosno_polje + "';");
