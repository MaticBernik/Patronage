...
class Pacient(models.Model):
    ...
    #   po defaultu mamo ime priimek email/username in geslo ze kr v Userju
    st_kartice = models.IntegerField(max_length=11, null=False)
    naslov = models.CharField(max_length=100, null=False)
    st_okolisa = models.IntegerField(max_length=10, null=False)
    telefonska_st = models.CharField(max_length=15, null=False)
    datum_rojstva = models.DateField(default=datetime.now() + timedelta(days=14)) # Zakaj je tu default now + 14?
    spol = models.CharField(max_length=1, null=False)
...
class Odvisnosti_oskrbovanec_skrbnik(models.Model):
    ...
    id_oskrbovanca = models.IntegerField(max_length=11, null=False)
