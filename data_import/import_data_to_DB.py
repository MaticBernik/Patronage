...
with open("testni_pacienti.csv","r") as patients_file: #encoding="utf8"
	...
	for line in patients_reader:
		...
		if line[0]=="da":
			...
			conn.execute("INSERT INTO patronazna_sluzba_app_pacient (ime,priimek,naslov,st_kartice,email,telefonska_st,spol,datum_rojstva,uporabniski_profil_id,okolis_id) VALUES (?,?,?,?,?,?,?,?,?,?)", (line[1], line[2], line[3], int(line[4]), line[5], line[6], line[7], datetime.strptime(line[8], '%d.%m.%Y'), id, id_okrozje));
		else:
			conn.execute("INSERT INTO patronazna_sluzba_app_pacient (ime,priimek,naslov,st_kartice,email,telefonska_st,spol,datum_rojstva,okolis_id) VALUES (?,?,?,?,?,?,?,?,?)", (line[1], line[2], line[3], int(line[4]), line[5], line[6], line[7], datetime.strptime(line[8], '%d.%m.%Y'), id_okrozje));
